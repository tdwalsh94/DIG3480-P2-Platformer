...
function SpikeDown(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.8, 0.8);
  this.wobble = Math.random() * Math.PI * 2;
}
SpikeDown.prototype.type = "spikeDown";
...
//Villian function
function Villian(pos, ch) {
  this.pos = pos;
  this.size = new Vector(.7, .8);
  if (ch == "G") {
    // Horizontal villian
...
Villian.prototype.type = "villian";
...
Villian.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
SpikeDown.prototype.act = function(step){
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "villian" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "spike" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "spikeDown" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
